...
{    
    ...
    Peripherals::Interrupt::RegisterInterrupt_Vct_Table(EventISR,static_cast<Peripherals::Interrupt::IRQn>(I2C1_EV_IRQHandler),0,0);
    Peripherals::Interrupt::RegisterInterrupt_Vct_Table(ErrorISR,static_cast<Peripherals::Interrupt::IRQn>(I2C1_ER_IRQHandler),0,0);
    return 0;
}
...
{
    while(m_RxDone != DONE);
    if( HAL_I2C_Master_Transmit_IT(&m_hi2c, DevAddress, pTxBuf, Len) == HAL_OK )
    {
        m_TxDone = BUSY;
        HAL_Delay(1); 
        return HAL_OK;
...
Status_t I2C_Master_IT::Read (uint16_t DevAddress, uint8_t *pRxBuf, uint16_t Len)
{
    ...
    if( HAL_I2C_Master_Receive_IT(&m_hi2c, DevAddress, pRxBuf, Len) == HAL_OK )
    {
        m_RxDone = BUSY;
        HAL_Delay(1); 
        return HAL_OK;
