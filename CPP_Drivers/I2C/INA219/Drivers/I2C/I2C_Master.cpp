...
Status_t I2C_Master::Xfer (uint16_t DevAddress, uint8_t *pTxBuf, uint16_t TxLen, uint8_t *pRxBuf, uint16_t RxLen)
{    
    ...
    {
        return HAL_I2C_Master_Receive(&m_hi2c, DevAddress, pRxBuf, RxLen, TIMEOUT);
    }
    else if( ((pTxBuf != nullptr) && (TxLen != 0U) ) && ( (pRxBuf == nullptr) && (RxLen == 0U) ) )
    {
        return HAL_I2C_Master_Transmit(&m_hi2c, DevAddress, pTxBuf, TxLen, TIMEOUT);
    }
    else if( ((pTxBuf != nullptr) && (TxLen != 0U) ) && ( (pRxBuf != nullptr) && (RxLen != 0U) ) )
    {
        if ( HAL_I2C_Master_Transmit(&m_hi2c, DevAddress, pTxBuf, TxLen, TIMEOUT) != HAL_OK )
            return HAL_ERROR;
        ...
        if ( HAL_I2C_Master_Receive(&m_hi2c, DevAddress, pRxBuf, RxLen, TIMEOUT) != HAL_OK )
            return HAL_ERROR;
         //HAL_Delay(50); 
        // Because above call is non-blocking we need to wait until TC flag is set.
       // while (!__HAL_I2C_GET_FLAG(&m_hi2c, I2C_FLAG_BTF));  
