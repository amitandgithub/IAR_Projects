...
void DisplayBuffer::DrawChar(unsigned char Row, unsigned char Col, const char aChar, uint8_t format)
{
    ...
    if(format == INVERSE)
    {
        ...
        {
            Buffer[Row*84 + Col*6 + i ] = ~ASCII1[aChar - 0x20][i];
        }        
    }
    else
    {
...
void DisplayBuffer::DrawBitmap(const char* Str, uint8_t format)
{
#if 0
    ...
    if(format == INVERSE)
    {
       for(Row = 0; Row < TOTAL_ROWS; Row++)
       {
           for(Chars = 0; Chars < NO_OF_CHAR_IN_LINE; Chars++)
           {
                for(i = 0; i < SIZE_OF_1_CHAR; i++)
                {
                    Buffer[Row*84 + Chars*6 + i] = ~ASCII1[Str[Row*84  + Chars*6 + i] - 0x20][i];
                }              
           }  
       }
