...
{
public:
    typedef StaticFunction<void()> MyCallback;
    ...
    GpioInput(Pin_t  Pin, GpioInput_Params_t Params = INPUT_PULL_UP , Intr_Event_Edge_t Edge = IT_ON_FALLING, ISR_t aISR = nullptr);
    ...
    inline void Read(uint16_t *pGpioInputBits) const;
    
    template <typename TFunc>
    void setHandler(TFunc&& func)
    {
        _GpioCallback = std::forward<TFunc>(func);
    }
...
private:
    ...
    static MyCallback               _GpioCallback;
};
...
inline void GpioInput::Read(uint16_t *pGpioInputBits) const
{
    *pGpioInputBits = _GPIOx->IDR;
}
