...
//for a font that is 6 pixels wide and 8 pixels high
  ...
  /*0x00,,{0x00, 0x00, 0x00, 0x00, 0x 00} // 20 // Padded to fix the bug, when try to print a, it prints b*/
...
void DisplayBuffer::DrawChar(unsigned char Line, unsigned char Char, const char aChar, uint8_t format)
{
    ...
    if(format == INVERSE)
    {
        ...
        {
           DisplayBufferMem.Struct.Line[Line%NO_OF_LINES_IN_SCREEN].Chars[Char%NO_OF_CHAR_IN_LINE][i] = ~ASCII1[aChar - 0x20][i];
        }        
    }
    else
    {
...
{
    unsigned char Row,Chars,i=0;
#if 0
    
        
    if(format == INVERSE)
    {
       for(Row = 0; Row < NO_OF_LINES_IN_SCREEN; Row++)
       {
    ...
    for(Row = 0; Row < NO_OF_LINES_IN_SCREEN; Row++)
    {
        for(Chars = 0; Chars < NO_OF_CHAR_IN_LINE; Chars++)
        {
                DrawChar(Row,Chars,Str[i++],format);         
        }  
    }        
}   
...
void DisplayBuffer::InvertRow(unsigned char Row) 
{
    for(uint32_t i=0; i< NO_OF_CHAR_IN_LINE; i++)
    {
        for(uint32_t j=0; j< SIZE_OF_1_CHAR; j++)
            DisplayBufferMem.Struct.Line[Row].Chars[i][j] = ~ DisplayBufferMem.Struct.Line[Row].Chars[i][j];
    }
}


void DisplayBuffer::ScroolText(unsigned char Row, Speed_t Speed, Dir_t Direction)
{
    (void)Row;
    (void)Speed;
    (void)Direction;
