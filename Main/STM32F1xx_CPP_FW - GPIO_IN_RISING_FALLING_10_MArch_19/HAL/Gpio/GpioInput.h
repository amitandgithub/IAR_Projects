...
class GpioInput : public Gpio, public Interrupt 
{
public:
    ...
    GpioInput(Pin_t  Pin, GpioInput_Params_t Params = INPUT_PULL_UP , Intr_Event_Edge_t Edge = IT_ON_FALLING, ISR_t rising_ISR = nullptr, ISR_t falling_ISR = nullptr);
...
private:
    static void RisingOrFallingISR(uint32_t ext_line);
...
private:
    ...
    static ISR_t                    _pRisingISR;
    static ISR_t                    _pFallingISR;    
    static Interrupt::ISR           _GPIO_ISRs[16];
};



inline bool GpioInput::Read() const
{
    return (_GPIOx->IDR) & _pin ? true : false;
}

inline void  GpioInput::RisingOrFallingISR(uint32_t ext_line)
{
    if((_GPIOx->IDR) & ext_line )
    {
        _pRisingISR();
    }
    else if (EXTI->FTSR & ext_line)
    {
        _pFallingISR();
    }
}
