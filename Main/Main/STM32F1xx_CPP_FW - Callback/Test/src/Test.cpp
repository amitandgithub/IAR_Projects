...
{
    static GpioInput Button(Gpio::A8, GpioInput::INPUT_PULL_DOWN, GpioInput::IT_ON_RISING, Button_ISR );
    Button.HwInit();
    C13_Led.HwInit();
    //Button.connectCallback(Button_ISR,0); 
    while(1)
    {
        if(Button.Read())
        {
...
{
    GpioInput::CallbackFunction cb = Button_ISR_cb;
    
    static GpioInput Button(Gpio::A8, GpioInput::INPUT_PULL_DOWN, GpioInput::IT_ON_RISING, cb );
    Button.HwInit();
    C13_Led.HwInit();
...
int Button_ISR_cb(int i)
{
    C13_Led.Blink(400,70+i);
    return 22;
}
